    ...
    public class ServicoEstoque : IServicoEstoque, IServicoEstoqueV2
    {
        ...
        public bool IncluirProduto(Produto Produto)
        {
            ...
            try
            {
                using (ProvedorEstoque database = new ProvedorEstoque())
                {
                    bool rlista = (from p in database.ProdutoEstoques where string.Equals(p.NomeProduto, Produto.NomeProduto) && string.Equals(p.NumeroProduto, Produto.NumeroProduto) select p).ToList().Count == 0;
                    if (rlista == true)
                    {
        ...
        public bool RemoverEstoque(string NumeroProduto, int Quantidade)
        {
            try
            {
                ...
                using (ProvedorEstoque database = new ProvedorEstoque())
                {
                    ...
                    // in to the operation
                    ProdutoEstoque stock = database.ProdutoEstoques.First(pi => pi.NumeroProduto == productID);
                    if (stock.EstoqueProduto > Quantidade)
                    {
                        stock.EstoqueProduto = stock.EstoqueProduto -  Quantidade;
                    }
                    else
                    {
                        stock.EstoqueProduto = 0;
                    }
